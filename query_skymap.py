...
def print_query_params(args, ra_center, dec_center, jd_trigger):
    ...
    if args.after_trigger or jd_trigger > 0:
        print(f"Only sources detected for the first time after {Time(jd_trigger, format='jd').iso} will be considered")
...
def check_clu_transients(sources_kowalski, clu_sources):
    ...
    for source in sources_kowalski:
        if source['name'] in list_clu_sources:
            clu_source = clu_sources[np.where(np.array(list_clu_sources) == source['name'])[0][0]]
            try:
    ...
    for slice_lim,i in zip(np.linspace(0,len(ra_center),slices)[:-1], np.arange(len(np.linspace(0,len(ra_center),slices)[:-1]))):
        ...
        "object_coordinates": {
         ...
         "catalogs": {
		     ...
		     "candidate.jdstarthist": {'$gt': jd_trigger, '$lt': jd_trigger + within_days}
		     },
                 "projection": {
                     "objectId": 1,
                     "candidate.rcid": 1,
...
def query_kowalski_coords(username, password, names):
    ...
    for n in names:
        ...
        source["candid"] = list(r["candid"] for r in results if r["objectId"] == n)[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ingest', dest='ingest', type=str2bool, required=False, 
    help='Use with caution! Ingest the candidates to the scanning page on the GROWTH marshal', default=False)
    parser.add_argument('--ingest-program', dest='ingest_program', type=str, required=False,
    help='Ingest the candidates to the scanning page of the given GROWTH marshal program',
    ...
    # Query kowalski
    sources_kowalski = query_kowalski(username, password, ra_center, dec_center, args.radius, jd_trigger, args.min_days, args.max_days, args.slices, args.ndethist_min, args.within_days, after_trigger=args.after_trigger)
    ...
    if args.ingest:
        ...
        sources_ingestion = list({"name": s["name"], "candid": s["candid"]} for s in sources_within if not (s["name"] in list(tbl_ingested["name"])))
        # Check if the sources were already ingested under another program name
        for s in sources_within:
            if s["name"] in list(x["name"] for x in sources_ingestion):
                continue
            else:
                if not (args.ingest_program in list(tbl_ingested["program"][tbl_ingested["name"] == s["name"]])):
                    print(f"Re-ingesting {s['name']} under {args.ingest_program}")
                    sources_ingestion.append({"name": s["name"], "candid": s["candid"]})
        if sources_ingestion == []:
            print("No sources to ingest")
        else:
            programidx = get_programidx(args.ingest_program, username_marshal, password_marshal)
            with open("ingested.csv", 'a') as ingested_file:
                for s in sources_ingestion:
                    r = requests.post(f"http://skipper.caltech.edu:8080/cgi-bin/growth/ingest_avro_id.cgi", auth=(username_marshal, password_marshal), data={'programidx': programidx, 'avroid': s['candid']})
                    try: 
                        r.raise_for_status()
                        print(f"Successfully ingested {s['name']} in {args.ingest_program}")
                        ingested_file.write(f"{s['name']}, {s['candid']}, {args.ingest_program}\n")
                    except requests.exceptions.HTTPError as e: 
                        print("Error ingesting {s['name']} in {args.ingest_program}")
                        print(e)
