...
#                                                      }
        ...
        print(result_data)
        try:
            if result_data['ZTF_alerts'] == []:
                continue
            keys_list = list(result_data['ZTF_alerts'].keys())
        ...
        for key in keys_list:
            all_info = result_data['ZTF_alerts'][key]
            ...
            for info in all_info:
                ...
                if (info['candidate']['jdendhist'] - info['candidate']['jdstarthist']) >= 72./24. and info['candidate']['ndethist'] <= 2. :
...
def main_function():
    ...
    parser.add_argument('--secrets_kowalski', dest='secrets_kowalski', type=str,
                        required=False,
                        help='username,password', default='secrets.csv')
    parser.add_argument('--secrets_marshal', dest='secrets_marshal', type=str,
                        required=False,
                        help='username,password', default='secrets.csv')
    parser.add_argument('--CLU', dest='clu', type=str2bool,
                        required=False,
                        help='username,password', default=False)
    parser.add_argument('--grb', dest='grb', type=str2bool,
                        required=False,
                        help='boolean for grb fits file', default=False)
    parser.add_argument('--total-number', dest='total_number', type=str2bool,
                        required=False,
                        help='Set True to return the total number of candidates at the end', default=False)
    ...
    if args.jd_trigger == -1 and (not args.grb):
        jd_trigger = Time(header['MJD-OBS'], format='mjd').jd
        
    elif args.jd_trigger == -1 and args.grb:
    	ind = args.skymap_filename.rfind('_bn')
    	date = args.skymap_filename[ind+3:ind+3+9]
    	h_temp = int(date[6:])/1000
    	YY,MM,DD,HH,mm =date[:2],date[2:4],date[4:6],int(24*h_temp),int(24*h_temp%1*60)
    	time = '20%.2d-%.2d-%.2dT%.2d:%.2d:00.00'%(int(YY),int(MM),int(DD),int(HH),int(mm))
    	
    	jd_trigger = Time(time,format='isot').jd
    ...
    # Read the secrets
    if args.secrets_kowalski == 'secrets.csv':
    	secrets = ascii.read('secrets.csv', format='csv')
    
    	username = secrets['kowalski_user'][0]
    	password = secrets['kowalski_pwd'][0]
    else:
    	username,password = args.secrets_kowalski.split(',')
    ...
    if args.secrets_marshal == 'secrets.csv':
    	secrets = ascii.read('secrets.csv', format='csv')
    
    	username = secrets['kowalski_user'][0]
    	password = secrets['kowalski_pwd'][0]
    else:
    	username,password = args.secrets_marshal.split(',')
...
#         print(sources_within)
        print('Number of sources', len(sources_within))
        return len(sources_within)
